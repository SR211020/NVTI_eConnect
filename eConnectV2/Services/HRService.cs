...
namespace eConnectV2.Services
{
    ...
    {
        DataTable usp_HR(HRViewModel model);
    }
    public class HRService : IHRService
    {
        ...
        public HRService(IOptions<ConnectionStrings> options)
        {
            _connectionStrings = options.Value;
        }
        public DataTable usp_HR(HRViewModel model)
        {
            try
            {
                Con = new SqlConnection(_connectionStrings.SQL_Connection);
                if (Con.State == ConnectionState.Closed) { Con.Open(); }
                Cmd = new SqlCommand("usp_HR", Con);
